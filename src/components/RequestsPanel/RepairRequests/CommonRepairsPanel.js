  ...
  if (isAllSelected) {
    if (category === "pending") {
      ...
      TableData = repairs.map((repair) => {
        return {
          id: repair.repair_quote_id,
          dataPoint: repair,
          cells: [
            repair.client_company || [t("general.not_applicable")],
            repair.client_request_work_order || [t("general.not_applicable")],
      ...
      TableData = repairs.map((repair) => {
        return {
          ...
          cells: [
            ...
            <GeneralBadge
              data={repair.is_urgent /*? "Yes" : "No"*/}
              colorTheme={repair.is_urgent === "Yes"? "badge-danger" : "badge-secondary"}
            />,
            capitalize(repair.status),
            ...(moment(repair.date_created).isValid()
              ? [moment(repair.date_created).format("YYYY-MM-DD")]
