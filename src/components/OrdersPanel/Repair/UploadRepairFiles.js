...
const UploadRepairFiles = ({
  ...
  const handleUploadData = () => {
    ...
    let data = { work_order: repair.work_order };
  ...
  const handleSubmit = (data) => {
    ...
    let uploadFilesAPI = `${Constants.ENDPOINT_PREFIX}/api-client/v1/Client/Repair/Upload/Files`;
  ...
  const refreshData = async () => {
    ...
    const callback = (res) => {
      ...
      for (let i in repairs) {
        if (repairs[i].work_order === repair.work_order) {
