 ...
 * @param {Boolean} noPrevState (optional) if defined, setImages can only accept new state as argument
 */
      ...
      Object.keys(event.target.files).forEach((key) => {
        ...
        if (event.target.files[key].name.toLowerCase().includes(".heic")) {
          heic2any({
            ...
            noPrevState ? setImages(file) : setImages((preImgs) => [...preImgs, file]);
          ...
          noPrevState
            ? setImages(event.target.files[key])
