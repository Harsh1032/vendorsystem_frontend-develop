...
const RequestProgress = ({ steps, contents, activeStep, layout = "horizontal" }) => {
  ...
  let stepImages = [robotStep1, robotStep2, robotStep3, robotStep4, robotStep5];
  if (steps.length > 5) {
    while (stepImages.length < steps.length) {
      let indexInsert = Math.floor(steps.length / 2);
      stepImages.splice(indexInsert, 0, robotStep3);
    }
  }
